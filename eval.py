...
def compute_metrics(sources, predictions, labels):
    ...
    """
    result_rouge = metric_rouge.compute(predictions=predictions, references=labels, use_stemmer=True)
    result_sari  = metric_sari.compute(sources=sources, predictions=predictions, references=labels)
    
    result_bert  = []
    for (pred,label) in zip(predictions, labels):
        result_bert_temp = metric_bertscore.compute(predictions=[pred]*len(label), references=label, lang="en")
...
dataset         = load_dataset('json', data_files=f'data/{DATASET_NAME}_multiple.json', field='train')
