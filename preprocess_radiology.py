...
def extract_data(df):
    return [{'input': clean_spaces(row['sentence']), 
                    'labels': [clean_spaces(row['simplified_sentence'])], 
                    'vocab': list(filter(lambda x: x!="", 
                                    list(map(clean_spaces, 
                                      row['difficult_words'].split(',')
                                        ))
                                    )),
                    'report_id': row['report_id']} for \
                    i, row in df.copy().iterrows()]    
...
df_full = df_full.groupby('report_id').aggregate({'sentence': lambda x: ' '.join(x),
                                   'simplified_sentence': lambda x: ' '.join(x),
                                   'difficult_words': lambda x: ' '.join(x)}).reset_index()
# Indiv means each training point is on the sentence level
df_indiv = df.copy()
df_indiv = df_indiv.loc[~df_indiv['sentence'].isnull() &\
                        ~df_indiv['simplified_sentence'].isnull()].reset_index(drop=True)
df_indiv['difficult_words'] = df_indiv['difficult_words'].fillna('').reset_index(drop=True)
...
output_json_full = {'train': extract_data(df_full_train), 
                    'test':  extract_data(df_full_test)}
output_json_indiv = {'train': extract_data(df_indiv_train), 
                     'test':  extract_data(df_indiv_test)}

# Filter out nulls from full
output_json_full['train'] = list(filter(lambda d: d['labels'][0]!='', output_json_full['train']))
output_json_full['test']  = list(filter(lambda d: d['labels'][0]!='', output_json_full['test']))

output_json_indiv['train'] = list(filter(lambda d: d['labels'][0]!='', output_json_indiv['train']))
output_json_indiv['test']  = list(filter(lambda d: d['labels'][0]!='', output_json_indiv['test']))
