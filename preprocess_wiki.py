...
if os.path.exists(f"misc/test_{input_dict['name']}_lst.pkl") and os.path.exists(f"misc/test_{input_dict['name']}_lst.pkl"):
    ...
    if args.dataset in ['radiology_indiv', 'radiology_full']:
        ...
        test_wiki_input_lst  = list(map(lambda s: replace_entities(s, bio=bio_flag), test_input_lst))
...
for train_idx, test_idx, filter_type in [input_lst_all, input_lst_some]:
    ...
    # Output the file
    write_json(output_json_indiv, f"data/{input_dict['name']}_context_{filter_type}.json")
    ...
    # Output the file
    write_json(output_json_mult, f"data/{input_dict['name']}_context_{filter_type}_multiple.json")